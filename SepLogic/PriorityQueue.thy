...
setup {* add_prfstep_prop_induction @{thm eq_pred.induct} *}
...
theorem eq_pred_parent1 [forward]: "eq_pred i (s1 k) \<Longrightarrow> i \<noteq> s1 k \<Longrightarrow> eq_pred i k"
  by (tactic {* auto2s_tac @{context} (PROP_INDUCT ("eq_pred i (s1 k)", [])) *})
theorem eq_pred_parent2 [forward]: "eq_pred i (s2 k) \<Longrightarrow> i \<noteq> s2 k \<Longrightarrow> eq_pred i k"
  by (tactic {* auto2s_tac @{context} (PROP_INDUCT ("eq_pred i (s2 k)", [])) *})
theorem eq_pred_cases [forward]:
  "eq_pred i j \<Longrightarrow> \<not>eq_pred (s1 i) j \<Longrightarrow> \<not>eq_pred (s2 i) j \<Longrightarrow> j = i \<or> j = s1 i \<or> j = s2 i" by auto2
theorem eq_pred_le [forward]: "eq_pred i j \<Longrightarrow> i \<le> j" by auto2
setup {* del_prfstep_thm @{thm eq_pred.induct} *}
...
theorem swap_zero_is_heap_partial1:
  ...
  by (tactic {* auto2s_tac @{context}
    (CHOOSE "xs', xs' = list_swap xs 0 (length xs - 1)" THEN
     OBTAIN "\<forall>i j. eq_pred i j \<longrightarrow> i < length xs - 1 \<longrightarrow> i \<noteq> 0 \<longrightarrow> j < length xs - 1 \<longrightarrow> xs' ! i \<le> xs' ! j" WITH
       CASE "j = 0") *})
...
theorem incr_is_heap_partial1:
  ...
  by (tactic {* auto2s_tac @{context}
    (CHOOSE "xs', xs' = list_update xs k v" THEN
     OBTAIN "\<forall>i j. eq_pred i j \<longrightarrow> i < length xs' \<longrightarrow> i \<noteq> k \<longrightarrow> j < length xs' \<longrightarrow> xs' ! i \<le> xs' ! j" WITH
       CASE "j = k") *})
...
val CASES_J =
  ...
  CASE "eq_pred (s1 i) j" THEN CASE "eq_pred (s2 i) j"
*}
...
theorem bubble_down1:
  ...
  by (tactic {* auto2s_tac @{context}
    (CHOOSE "xs', xs' = list_swap xs k (s1 k)" THEN
     OBTAIN "\<forall>i j. eq_pred i j \<longrightarrow> i < length xs \<longrightarrow> i \<noteq> s1 k \<longrightarrow> j < length xs \<longrightarrow> xs' ! i \<le> xs' ! j" WITH
...
theorem bubble_down2: "s2 k < length xs \<Longrightarrow> xs ! (s1 k) > xs ! (s2 k) \<Longrightarrow> xs ! k > xs ! (s2 k) \<Longrightarrow>
  ...
  by (tactic {* auto2s_tac @{context}
    (CHOOSE "xs', xs' = list_swap xs k (s2 k)" THEN
     OBTAIN "\<forall>i j. eq_pred i j \<longrightarrow> i < length xs \<longrightarrow> i \<noteq> s2 k \<longrightarrow> j < length xs \<longrightarrow> xs' ! i \<le> xs' ! j" WITH
...
theorem bubble_down3 [forward]:
  ...
  by (tactic {* auto2s_tac @{context}
   (OBTAIN "\<forall>i j. eq_pred i j \<longrightarrow> i < length xs \<longrightarrow> j < length xs \<longrightarrow> xs ! i \<le> xs ! j" WITH CASES_J) *})+
...
setup {* del_prfstep_thm @{thm eq_pred.induct} *}
setup {* fold add_forward_prfstep @{thms eq_pred_p(1,2)} *}
setup {* add_forward_prfstep_cond @{thm eq_pred_p(3)} [with_term "par ?i"] *}
...
theorem bubble_up1 [forward]:
  ...
  by (tactic {* auto2s_tac @{context}
    (CHOOSE "xs', xs' = list_swap xs k (par k)" THEN
     OBTAIN "\<forall>i j. eq_pred i j \<longrightarrow> i < length xs \<longrightarrow> j < length xs \<longrightarrow> j \<noteq> par k \<longrightarrow> xs' ! i \<le> xs' ! j" WITH
...
theorem bubble_up2 [forward]:
  ...
  by (tactic {* auto2s_tac @{context}
   (OBTAIN "\<forall>i j. eq_pred i j \<longrightarrow> i < length xs \<longrightarrow> j < length xs \<longrightarrow> xs ! i \<le> xs ! j" WITH
...
theorem desc_is_heap_partial2:
  ...
  by (tactic {* auto2s_tac @{context}
   (CHOOSE "xs', xs' = list_update xs k v" THEN
    OBTAIN "\<forall>i j. eq_pred i j \<longrightarrow> i < length xs' \<longrightarrow> j < length xs' \<longrightarrow> j \<noteq> k \<longrightarrow> xs' ! i \<le> xs' ! j" WITH
      CASE "i = k") *})
setup {* add_forward_prfstep_cond @{thm desc_is_heap_partial2} [with_term "list_update ?xs ?k ?v"] *}

setup {* fold del_prfstep_thm @{thms eq_pred_p} *}
...
theorem mset_butlast [rewrite]: "length xs > 0 \<Longrightarrow> mset (butlast xs) = mset xs - {# last xs #}"
  by (metis add_diff_cancel_right' append_butlast_last_id cancel_comm_monoid_add_class.diff_cancel
      insert_DiffM2 length_greater_0_conv list.size(3) mset.simps(2) multi_member_last
      size_eq_0_iff_empty size_mset union_code)
