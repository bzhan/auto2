...
theory RBT
imports SepAuto "../Mapping" "../DataStrs/RBT_Base"
begin
...
lemma btree_leaf: "btree Leaf p \<Longrightarrow>\<^sub>A \<up>(p = None)" by auto2
setup {* add_forward_ent_prfstep_cond @{thm btree_leaf} [K ShadowFirst] *}

lemma btree_split_iff1: "btree t None \<Longrightarrow>\<^sub>A \<up>(t = Leaf)" by auto2
...
lemma btree_constr_ent:
  "btree lt lp * btree rt rp * p \<mapsto>\<^sub>r Node lp c k v rp \<Longrightarrow>\<^sub>A btree (pre_rbt.Node lt c k v rt) (Some p)" by auto2
...
setup {* add_forward_prfstep_cond @{thm rbt_is_not_leaf} [with_cond "?t \<noteq> pre_rbt.Node ?lt ?c ?k ?v ?rt"] *}
...
setup {* add_rewrite_rule @{thm rbt_set_assn_def} *}
...
setup {* add_rewrite_rule @{thm rbt_map_assn_def} *}
