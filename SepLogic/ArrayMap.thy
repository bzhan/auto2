...
definition amap_of_list :: "'a option list \<Rightarrow> (nat, 'a) map" where
  "amap_of_list xs = Map (\<lambda>i. if i < length xs then xs ! i else None)"
setup {* add_rewrite_rule @{thm amap_of_list_def} *}

theorem amap_of_listI [backward]:
...
setup {* add_rewrite_rule @{thm amap.simps} *}

theorem amap_prec [sep_prec_thms]:
  "h \<Turnstile> amap m p * F1 \<Longrightarrow> h \<Turnstile> amap m' p * F2 \<Longrightarrow> m = m'" by auto2

definition amap_new :: "nat \<Rightarrow> ('a::heap) array_map Heap" where
  "amap_new n = do {
