...
definition per_union :: "('a \<times> 'a) set \<Rightarrow> 'a \<Rightarrow> 'a \<Rightarrow> ('a \<times> 'a) set" where [rewrite]:
  "per_union R a b = R \<union> { (x,y). (x,a)\<in>R \<and> (y,b)\<in>R } \<union> { (y,x). (x,a)\<in>R \<and> (y,b)\<in> R}"
...
lemma per_union_memI2 [backward]:
  "(x, a) \<in> R \<Longrightarrow> (y, b) \<in> R \<Longrightarrow> (x, y) \<in> per_union R a b" by (simp add: per_union_def)

lemma per_union_memI3 [backward]:
  "(x, a) \<in> R \<Longrightarrow> (y, b) \<in> R \<Longrightarrow> (y, x) \<in> per_union R a b" by (simp add: per_union_def)

lemma per_union_memD:
  "(x, y) \<in> per_union R a b \<Longrightarrow> (x, y) \<in> R \<or> ((x, a) \<in> R \<and> (y, b) \<in> R) \<or> ((y, a) \<in> R \<and> (x, b) \<in> R)"
