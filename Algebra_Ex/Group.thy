...
setup {* fold add_property_update @{thms monoid_props(1,2)} *}
...
lemma Units_one_closed: fixes G (structure) shows
  ...
  by (tactic {* auto2s_tac @{context} (OBTAIN "\<one> \<otimes> \<one> = \<one>") *})
...
lemma Units_m_closed: fixes G (structure) shows
  ...
  by (tactic {* auto2s_tac @{context}
    (CHOOSE "x', elt x' \<and> x' \<otimes> x = \<one> \<and> x \<otimes> x' = \<one>" THEN
     ...
     OBTAIN "(y' \<otimes> x') \<otimes> (x \<otimes> y) = \<one>" THEN
     OBTAIN "(x \<otimes> y) \<otimes> (y' \<otimes> x') = \<one>") *})
...
lemma inv_unique [forward]: fixes G (structure) shows
  ...
  by (tactic {* auto2s_tac @{context} (OBTAIN "y = y \<otimes> (x \<otimes> y')") *})
...
theorem monoid_inv_prop: fixes G (structure) shows
  ...
  by (tactic {* auto2s_tac @{context}
    (OBTAIN "\<exists>!y. elt y \<and> y \<otimes> x = \<one> \<and> x \<otimes> y = \<one>") *})
...
lemma Units_l_cancel [forward]: fixes G (structure) shows
  ...
  by (tactic {* auto2s_tac @{context} (OBTAIN "(inv x \<otimes> x) \<otimes> y = (inv x \<otimes> x) \<otimes> z") *})
...
lemma Units_r_cancel [forward]: fixes G (structure) shows
  ...
  by (tactic {* auto2s_tac @{context} (OBTAIN "y \<otimes> (x \<otimes> inv x) = z \<otimes> (x \<otimes> inv x)") *})
...
lemma Units_inv_comm [backward2]: fixes G (structure) shows
  ...
  by (tactic {* auto2s_tac @{context} (OBTAIN "x \<otimes> y \<otimes> x = x \<otimes> \<one>") *})

lemma monoid_set_not_empty: fixes G (structure) shows
  "monoid G \<Longrightarrow> \<exists>x. elt x"
  by (tactic {* auto2s_tac @{context} (OBTAIN "elt \<one>") *})
...
theorem groupI [backward]: fixes G (structure) shows
  ...
  by (tactic {* auto2s_tac @{context}
    ...
    (OBTAIN "\<forall>x y z. elt x \<longrightarrow> elt y \<longrightarrow> elt z \<longrightarrow> x \<otimes> y = x \<otimes> z \<longrightarrow> y = z" WITH
      (CHOOSE "x', elt x' \<and> x' \<otimes> x = \<one>" THEN
       OBTAIN "x' \<otimes> x \<otimes> y = x' \<otimes> x \<otimes> z") THEN
    (* Key idea is that y \<otimes> x = \<one> implies x \<otimes> y = \<one> *)
    (OBTAIN "\<forall>x. elt x \<longrightarrow> x \<in> Units G" WITH
      (CHOOSE "y, elt y \<and> y \<otimes> x = \<one>" THEN
       OBTAIN "y \<otimes> x \<otimes> y = y \<otimes> \<one>"))) *})
setup {* del_prfstep_thm @{thm group_def} #>
  add_forward_prfstep_cond (equiv_forward_th @{thm group_def}) [with_term "?G"] *}

theorem group_to_monoid: "group G \<Longrightarrow> monoid G" by auto2
setup {* add_property_update @{thm group_to_monoid} *}
...
theorem group_inv_prop: fixes G (structure) shows
  "group G \<Longrightarrow> elt x \<Longrightarrow> elt (inv x) \<and> inv x \<otimes> x = \<one> \<and> x \<otimes> inv x = \<one>" by auto2
setup {* add_property_update (conj_left_th @{thm group_inv_prop}) *}
setup {* add_rewrite_rule (conj_left_th (conj_right_th @{thm group_inv_prop})) *}
setup {* add_rewrite_rule (conj_right_th (conj_right_th @{thm group_inv_prop})) *}
...
theorem inv_equality [backward]: fixes G (structure) shows
  ...
  by (tactic {* auto2s_tac @{context} (OBTAIN "inv x \<otimes> x = \<one>") *})
...
lemma subgroup_sel:
  "elt\<^bsub>subgroup H G\<^esub> x \<longleftrightarrow> x \<in> H"
...
setup {* add_forward_prfstep_cond (equiv_forward_th @{thm subgroup_sel(1)}) [with_term "?x", with_term "is_subgroup ?H ?G"] *}
setup {* add_forward_prfstep_cond (equiv_backward_th @{thm subgroup_sel(1)}) [with_term "is_subgroup ?H ?G"] *}
...
lemma subgroup_is_group: "is_subgroup H G \<Longrightarrow> group (subgroup H G)"
  by (tactic {* auto2s_tac @{context}
    (CHOOSE "\<H>, \<H> = subgroup H G" THEN
     OBTAIN "monoid \<H>" THEN
     OBTAIN "\<forall>x. elt\<^bsub>\<H>\<^esub> x \<longrightarrow> (\<exists>y. elt\<^bsub>\<H>\<^esub> y \<and> y \<otimes>\<^bsub>\<H>\<^esub> x = \<one>\<^bsub>\<H>\<^esub>)" WITH
      (OBTAIN "inv\<^bsub>G\<^esub> x \<otimes>\<^bsub>\<H>\<^esub> x = \<one>\<^bsub>\<H>\<^esub>")) *})

theorem subgroup_non_empty [resolve]: "is_subgroup H G \<Longrightarrow> H \<noteq> {}"
  by (tactic {* auto2s_tac @{context} (OBTAIN "\<one>\<^bsub>G\<^esub> \<in> H") *})
...
lemma DirProd_group: "group G \<Longrightarrow> group H \<Longrightarrow> group (G \<times>\<times> H)"
  by (tactic {* auto2s_tac @{context}
    (CHOOSE "A, A = G \<times>\<times> H" THEN
     OBTAIN "monoid A" THEN
     OBTAIN "\<forall>x. elt\<^bsub>A\<^esub> x \<longrightarrow> (\<exists>y. elt\<^bsub>A\<^esub> y \<and> y \<otimes>\<^bsub>A\<^esub> x = \<one>\<^bsub>A\<^esub>)" WITH
       OBTAIN "(inv\<^bsub>G\<^esub> (fst x), inv\<^bsub>H\<^esub> (snd x)) \<otimes>\<^bsub>A\<^esub> x = \<one>\<^bsub>A\<^esub>") *})
...
lemma hom_one_closed [rewrite]: fixes G (structure) shows
  ...
  by (tactic {* auto2s_tac @{context} (OBTAIN "h\<langle>\<one> \<otimes> \<one>\<rangle> = h\<langle>\<one>\<rangle> \<otimes>\<^bsub>H\<^esub> \<one>\<^bsub>H\<^esub>") *})
...
lemma hom_inv_closed [rewrite]: fixes G (structure) shows
  ...
  by (tactic {* auto2s_tac @{context} (OBTAIN "h\<langle>inv x \<otimes> x\<rangle> = \<one>\<^bsub>H\<^esub>") *})
...
lemma iso_sym: "group G \<Longrightarrow> group H \<Longrightarrow> h \<in> G \<cong> H \<Longrightarrow> inv_fun\<^bsub>G\<^esub> h \<in> H \<cong> G"
  by (tactic {* auto2s_tac @{context}
    (CHOOSE "h', h' = inv_fun\<^bsub>G\<^esub> h" THEN
     OBTAIN "\<forall>x y. elt\<^bsub>H\<^esub> x \<longrightarrow> elt\<^bsub>H\<^esub> y \<longrightarrow> h' \<langle>x \<otimes>\<^bsub>H\<^esub> y\<rangle> = (h'\<langle>x\<rangle>) \<otimes>\<^bsub>G\<^esub> (h'\<langle>y\<rangle>)" WITH
      (OBTAIN "h \<langle>h' \<langle>x \<otimes>\<^bsub>H\<^esub> y\<rangle>\<rangle> = x \<otimes>\<^bsub>H\<^esub> y" THEN
       OBTAIN "h \<langle>h'\<langle>x\<rangle> \<otimes>\<^bsub>G\<^esub> h'\<langle>y\<rangle>\<rangle> = x \<otimes>\<^bsub>H\<^esub> y")) *})
