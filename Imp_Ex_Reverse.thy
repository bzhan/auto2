...
setup {* del_prfstep_thm @{thm swap.simps}
      #> fold add_rewrite_rule [@{thm swap_pointwise}, @{thm swap_length}]
...
theorem rev_induct': "(\<forall>a i j. (i < j \<longrightarrow> P a (i + 1) (j - 1)) \<longrightarrow> P a i j) \<Longrightarrow> P (a::'a\<Colon>heap array) (i::nat) (j::nat)"
  apply (induct rule: rev.induct) by blast
setup {* add_prfstep_imp_induction @{term_pat "rev ?a ?i ?j"} @{thm rev_induct'} *}

(* Properties of rev. *)
lemma rev_length: "effect (rev a i j) h h' r \<Longrightarrow> Array.length h a = Array.length h' a" by auto2
setup {* add_rewrite_rule @{thm rev_length} *}

theorem reduce_minus_1: "(k::nat) \<ge> i + 1 \<Longrightarrow> j - 1 - (k - (1 + i)) = j - (k - i)" by simp
setup {* add_rewrite_rule @{thm reduce_minus_1} *}

lemma rev_pointwise: "effect (rev a i j) h h' r \<Longrightarrow>
  Array.get h' a ! k = (if k < i then Array.get h a ! k else if j < k then Array.get h a ! k
                        else Array.get h a ! (j - (k - i)))" by auto2

lemma rev_succeed': "i < Array.length h a \<and> j < Array.length h a \<Longrightarrow> success (rev a i j) h" by auto2
setup {* add_backward_prfstep @{thm rev_succeed'} *}
lemma rev_succeed: "success (rev a 0 (Array.length h a - 1)) h" by auto2

setup {* del_prfstep_thm @{thm rev.simps} #> del_prfstep_thm @{thm rev_induct'}
      #> add_rewrite_rule @{thm rev_pointwise} *}
