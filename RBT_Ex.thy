...
setup {* fold add_rewrite_rule [@{thm if_not_Leaf}, @{thm if_R}, @{thm if_B}] *}
...
theorem bd_inv_elimR: "bd_inv (Node l R a r) \<Longrightarrow> black_depth (Node l R a r) = black_depth l" by auto2
theorem bd_inv_elimB: "bd_inv (Node l B a r) \<Longrightarrow> black_depth (Node l B a r) = black_depth l + 1" by auto2
...
setup {* fold add_rewrite_rule [@{thm bd_inv_elimR}, @{thm bd_inv_elimB}] *}
setup {* add_forward_prfstep_cond @{thm bd_inv_intro} [with_term "Node ?l ?c ?a ?r"] *}

subsection {* is_rbt is recursive *}

theorem is_rbt_rec: "is_rbt (Node l c a r) \<Longrightarrow> is_rbt l \<and> is_rbt r"
...
setup {* fold add_rewrite_rule (@{thms balanceR.simps(1,4-5)} @ @{thms balanceR_def'}) *}
...
setup {* fold add_rewrite_rule (@{thms balance.simps(1,4)} @ @{thms balance_def'}) *}
...
theorem rbt_set_balanceR: "rbt_set (balanceR t) = rbt_set t"
  by (tactic {* auto2s_tac @{context} balanceR_cases *})
setup {* add_rewrite_rule @{thm rbt_set_balanceR} *}
theorem rbt_set_balance: "rbt_set (balance t) = rbt_set t"
  by (tactic {* auto2s_tac @{context} balance_cases *})
setup {* add_rewrite_rule @{thm rbt_set_balance} *}
...
theorem cl_inv_B: "cl t = B \<Longrightarrow> cl_inv' t \<Longrightarrow> cl_inv t" by auto2
theorem cl_inv_R: "cl_inv' (Node l R x r) \<Longrightarrow> cl l = B \<Longrightarrow> cl r = B \<Longrightarrow> cl_inv (Node l R x r)" by auto2
theorem cl_inv_imp: "cl_inv t \<Longrightarrow> cl_inv' t" by (tactic {* auto2s_tac @{context} (CASE "cl t = R") *})
setup {* add_backward2_prfstep @{thm cl_inv_B} #> add_forward_prfstep @{thm cl_inv_imp} *}
setup {* fold add_forward_prfstep [@{thm cl_inv_B}, @{thm cl_inv_R}] *}

subsection {* balance function preserves cl_inv *}

theorem balanceR: "cl_inv l \<Longrightarrow> cl_inv' r \<Longrightarrow> cl_inv (balanceR (Node l B a r))"
...
theorem balance1: "cl_inv' l \<Longrightarrow> cl_inv r \<Longrightarrow> cl_inv (balance (Node l B a r))"
  by (tactic {* auto2s_tac @{context} balance_node_cases *})
theorem balance2: "cl_inv l \<Longrightarrow> cl_inv' r \<Longrightarrow> cl_inv (balance (Node l B a r))"
  by (tactic {* auto2s_tac @{context} balance_node_cases *})
...
theorem balanceR_non_Leaf: "balanceR (Node l c a r) \<noteq> Leaf"
...
theorem rbt_set_ins: "rbt_set (ins x t) = {x} \<union> rbt_set t" by auto2
setup {* add_rewrite_rule @{thm rbt_set_ins} *}
...
theorem rbt_set_makeBlack: "rbt_set (makeBlack t) = rbt_set t" by auto2
setup {* add_rewrite_rule @{thm rbt_set_makeBlack} *}
...
theorem balanceR_as_rotate1: "cl r = R \<Longrightarrow> cl (lsub r) = R \<Longrightarrow>
  base_tree (balanceR (Node l B x r)) = rotateL (rotateR_at_R (base_tree (Node l B x r)))" by auto2
theorem balanceR_as_rotate2: "cl r = R \<Longrightarrow> cl (lsub r) = B \<Longrightarrow> cl (rsub r) = R \<Longrightarrow>
  base_tree (balanceR (Node l B x r)) = rotateL (base_tree (Node l B x r))" by auto2
theorem balance_as_rotate1: "cl l = R \<Longrightarrow> cl (lsub l) = R \<Longrightarrow>
  base_tree (balance (Node l B x r)) = rotateR (base_tree (Node l B x r))" by auto2
theorem balance_as_rotate2: "cl l = R \<Longrightarrow> cl (lsub l) = B \<Longrightarrow> cl (rsub l) = R \<Longrightarrow>
  base_tree (balance (Node l B x r)) = rotateR (rotateL_at_L (base_tree (Node l B x r)))" by auto2
setup {* fold add_rewrite_rule [@{thm balanceR_as_rotate1}, @{thm balanceR_as_rotate2},
  @{thm balance_as_rotate1}, @{thm balance_as_rotate2}] *}
...
theorem rbt_set_base: "rbt_set t = tree_set (base_tree t)" by auto2
setup {* add_rewrite_rule_bidir @{thm rbt_set_base} *}
